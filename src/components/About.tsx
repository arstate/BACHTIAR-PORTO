          ...
          >
            ...
            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="glass-card p-8 md:p-10 relative overflow-hidden">
              ...
              <p className="text-lg md:text-xl text-white/70 leading-relaxed font-light">
                Allowing the visuals to speak for themselves is the foundation of my work. Instead of being limited by production silos, I control the entire process—capturing the essence on camera and refining the final rhythm in post-production. Backed by an understanding of <span className="text-white font-medium">Graphic Design</span> and my track record with <span className="text-white font-medium">Arstate.Cinema</span> since 2020, I forge raw concepts into flawless visual narratives. The process might be unusual, but my final work does all the talking.
              </p>
            </motion.div>

            {/* Stats Row */}
            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="grid grid-cols-3 gap-4">
              {[
                { value: "6+", label: "Years Active" },
                { value: "200+", label: "Events Covered" },
                { value: "20+", label: "Brand Clients" },
              ].map((stat, i) => (
                <div key={i} className="glass-card p-5 text-center hover:border-white/20 transition-colors group">
                  <div className="text-3xl md:text-4xl font-bold text-white group-hover:text-blue-400 transition-colors">{stat.value}</div>
